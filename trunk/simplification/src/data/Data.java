...
public class Data {
     ...
     * type = {1, 0} : 1 - considering date and time, 0 - consideting only time
     *
     * returns hashmap : route id -> arraylist of datapoints within provided time period
     *
     * @param tbName
     * @param timeFrom
     * @param timeTo
    ...
    public HashMap getDefinedTrajectories(String tbName, String timeFrom, String timeTo, int type) throws Exception {
        ...
        if (type == 1) {
            select = "select * from simpl_get_defined_routes('" + tbName + "', '" + timeFrom + "', '" + timeTo + "') as p(routeid bigint)";
        }
        else if (type == 0) {
