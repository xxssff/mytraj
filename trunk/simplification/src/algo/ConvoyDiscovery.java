...
public class ConvoyDiscovery {
    ...
    static HashMap<Integer, ArrayList<DataSegment>> hmSeg;
     ...
     * @return
     */
    private static List<Convoy> cutsFilter(List<LocalDateTime> times, int minPts, int k, double e) {
        int lambda = 86400; // for elk data: 101001600
        List<Convoy> V = new ArrayList<Convoy>();
        List<Convoy> Vcand = new ArrayList<Convoy>();
        for (LocalDateTime ldt : times) {
            List<Convoy> Vnext = new ArrayList<Convoy>();
            ArrayList<MovingObject> mos = getExpectedSegments(ldt);
            if (mos.size() < minPts) {
                continue;
            }
            DBScan.doDBScan(mos, e, minPts, ldt); //change with segment-DBscan
            HashMap<Integer, Cluster> C = new HashMap<Integer, Cluster>();
            fillClusters(C, mos);
            for (Convoy v : V) {
                v.assigned = false;
                ...
                if (!v.assigned && Seconds.secondsBetween(v.startTime, v.endTime).getSeconds() >= k) {
                    Vcand.add(v);
                }
            }
            for (Integer i : C.keySet()) {
    ...
    private static ArrayList<MovingObject> getExpectedSegments(LocalDateTime ldt) {
        ArrayList<MovingObject> res = new ArrayList<MovingObject>();
        for (Integer i : hmSeg.keySet()) {
            ArrayList<DataSegment> segments = hmSeg.get(i);
            if (segments.get(0).dateTimeSt.isBefore(ldt)
                    && segments.get(segments.size() - 1).dateTimeEn.isAfter(ldt)) {
                DataSegment ds = getExpectedSegmentOneRoute(i, ldt);
                MovingObject mo = new MovingObject(i, ds);
                if (mo != null) {
                    res.add(mo);
                }
            }
    ...
    private static List<Convoy> cutsRefinement(List<Convoy> Vcand, List<LocalDateTime> times, int minPts, int k, double e) throws Exception {
        for (Convoy v : Vcand) {
